...
// Start exec's the node passing the ID and service as comptcd line arguments
...
// execComptcd returns a comptcd which runs the node locally by exec'ing
// the current binary but setting argv[0] to "p2p-node" so that the child
// runs execP2PNode
func (n *ExecNode) execComptcd() *exec.Cmd {
	return &exec.Cmd{
		Path: reexec.Self(),
