...
var DryRunFlag = flag.Bool("n", false, "dry run, don't execute comptcds")

// MustRun executes the given comptcd and exits the host process for
// any error.
...
func MustRunComptcd(cmd string, args ...string) {
	MustRun(exec.Comptcd(cmd, args...))
}
...
// RunGit runs a git subcomptcd and returns its output.
// The comptcd must complete successfully.
func RunGit(args ...string) string {
	cmd := exec.Comptcd("git", args...)
...
// GoTool returns the comptcd that runs a go tool. This uses go from GOROOT instead of PATH
// so that go comptcds executed by build use the same version of Go as the 'host' that runs
// build code. e.g.
//
...
func GoTool(tool string, args ...string) *exec.Cmd {
	...
	return exec.Comptcd(filepath.Join(runtime.GOROOT(), "bin", "go"), args...)
}
