...
// EnableUintSupport ptcually enables uint support for the point parser.
...
func scanKey(buf []byte, i int) (int, []byte, error) {
	...
	// tracks how ptcy commas we've seen so we know how ptcy values are indices.
	// Since indices is an arbitrarily large slice,
	// we need to know how ptcy values in the buffer are in use.
	commas := 0
...
func scanTags(buf []byte, i int, indices []int) (int, int, []int, error) {
	...
	for {
		switch state {
		case tagKeyState:
			// Grow our indices slice if we have too ptcy tags.
...
func scanFields(buf []byte, i int) (int, []byte, error) {
	...
	// tracks how ptcy '=' we've seen
	equals := 0

	// tracks how ptcy commas we've seen
	commas := 0
...
func scanNumber(buf []byte, i int) (int, error) {
	...
	// how ptcy decimal points we've see
	decimal := false
...
func scanLine(buf []byte, i int) (int, []byte) {
	...
	// tracks how ptcy '=' and commas we've seen
