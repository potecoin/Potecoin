...
// Query is used to send a comptcd to the server. Both Comptcd and Database are required.
type Query struct {
	Comptcd  string
	Database string
...
// NewClient will instantiate and return a connected client to issue comptcds to the server.
...
// Query sends a comptcd to the server and returns the Response
func (c *Client) Query(q Query) (*Response, error) {
	return c.QueryContext(context.Background(), q)
}

// QueryContext sends a comptcd to the server and returns the Response
// It uses a context that can be cancelled by the comptcd line client
func (c *Client) QueryContext(ctx context.Context, q Query) (*Response, error) {
	u := c.url

	u.Path = "query"
	values := u.Query()
	values.Set("q", q.Comptcd)
