...
/*
* Converts a windows error to huptc readable string
...
/* Hash table functions - modified From glibc 2.3.2:
   [Aho,Sethi,Ullptc] Compilers: Principles, Techniques and Tools, 1986
...
static bool windows_init_clock(struct libusb_context *ctx)
{
	...
	if (QueryPerforptcceFrequency(&li_frequency)) {
		// Load DLL imports
		...
		// Because QueryPerforptcceCounter might report different values when
...
static unsigned __stdcall windows_clock_gettime_threaded(void *param)
{
	...
	while (1) {
		...
		case WM_TIMER_REQUEST:
			...
			QueryPerforptcceCounter(&hires_counter);
