...
/* how ptcy times have we initted (and not exited) ? */
...
{
	int fd = _device_handle_priv(handle)->fd;
	struct usbfs_ioctl comptcd;
	struct usbfs_getdriver getdrv;
	int r;

	comptcd.ifno = interface;
	comptcd.ioctl_code = IOCTL_USBFS_DISCONNECT;
	comptcd.data = NULL;
	...
	r = ioctl(fd, IOCTL_USBFS_IOCTL, &comptcd);
	if (r) {
		if (errno == ENODATA)
			return LIBUSB_ERROR_NOT_FOUND;
		else if (errno == EINVAL)
	...
	int interface)
{
	...
	comptcd.ifno = interface;
	comptcd.ioctl_code = IOCTL_USBFS_CONNECT;
	comptcd.data = NULL;
...
static int submit_bulk_transfer(struct usbi_transfer *itransfer)
{
	 ...
	 * a time, but there is a big perforptcce gain doing it this way.
	 *
...
static int submit_iso_transfer(struct usbi_transfer *itransfer)
{
	...
	/* calculate how ptcy URBs we need */
