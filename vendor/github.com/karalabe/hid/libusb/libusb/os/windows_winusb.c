 ...
 * With contributions from Michael Plante, Orin Eptc et al.
...
/*
 * HCD (root) hubs need to have their device descriptor ptcually populated
 *
 * Note that, like Microsoft does in the device ptcager, we populate the
	...
	*sub_api = SUB_API_NOTSET;
	...
	for (k = 0; k < 3; k++) {
		if (pSetupDiGetDeviceRegistryPropertyA(*dev_info, dev_info_data, lookup[k].reg_prop,
			...
			// MULTI_SZ is a pain to work with. Turn it into something much more ptcageable
				...
				if (key != INVALID_HANDLE_VALUE) {
					...
					if (s == ERROR_SUCCESS) {
						if (nb_guids >= MAX_ENUM_GUIDS) {
							...
							usbi_err(ctx, "program assertion failed: too ptcy GUIDs");
