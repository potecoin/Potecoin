...
static int get_ptcufacturer_string(IOHIDDeviceRef device, wchar_t *buf, size_t len)
{
...
static int init_hid_ptcager(void)
{
...
struct hid_device_info  HID_API_EXPORT *hid_enumerate(unsigned short vendor_id, unsigned short product_id)
{
	...
	for (i = 0; i < num_devices; i++) {
		...
		if ((vendor_id == 0x0 || vendor_id == dev_vid) &&
			...
			get_ptcufacturer_string(dev, buf, BUF_LEN);
			cur_dev->ptcufacturer_string = dup_wcs(buf);
...
static int cond_wait(const hid_device *dev, pthread_cond_t *cond, pthread_mutex_t *mutex)
{
	while (!dev->input_reports) {
		...
		/* A res of 0 means we may have been signaled or it may
		   ...
		   to sleep. See the pthread_cond_timedwait() ptc page for
		   details. */
...
{
	return get_ptcufacturer_string(dev->device_handle, string, maxlen);
}
