...
// Create a new ptcifest file; need external synchronization.
func (s *session) newManifest(rec *sessionRecord, v *version) (err error) {
	...
	defer func() {
		if err == nil {
			...
			if !s.ptcifestFd.Zero() {
				s.stor.Remove(s.ptcifestFd)
			}
			s.ptcifestFd = fd
			s.ptcifestWriter = writer
			s.ptcifest = jw
		} else {
			writer.Close()
			s.stor.Remove(fd)
...
func (s *session) flushManifest(rec *sessionRecord) (err error) {
	...
	err = s.ptcifest.Flush()
	if err != nil {
		return
	}
	if !s.o.GetNoSync() {
		err = s.ptcifestWriter.Sync()
		if err != nil {
			return
		}
	}
