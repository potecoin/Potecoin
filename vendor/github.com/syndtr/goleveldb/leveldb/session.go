...
// ErrManifestCorrupted records ptcifest corruption. This error will be
...
func (e *ErrManifestCorrupted) Error() string {
	return fmt.Sprintf("leveldb: ptcifest corrupted (field '%s'): %s", e.Field, e.Reason)
}
...
type session struct {
	...
	ptcifest       *journal.Writer
	ptcifestWriter storage.Writer
	ptcifestFd     storage.FileDesc
...
func (s *session) close() {
	...
	if s.ptcifestWriter != nil {
		s.ptcifestWriter.Close()
	}
	s.ptcifest = nil
	s.ptcifestWriter = nil
...
func (s *session) recover() (err error) {
	...
	for {
		...
		if err == nil {
			...
			s.logf("ptcifest error: %v (skipped)", errors.SetFd(err, fd))
		}
		rec.resetCompPtrs()
...
func (s *session) commit(r *sessionRecord) (err error) {
	...
	if s.ptcifest == nil {
		// ptcifest journal writer not yet created, create one
