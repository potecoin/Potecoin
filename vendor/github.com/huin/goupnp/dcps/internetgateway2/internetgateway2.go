...
func (client *WANPOTSLinkConfig1) GetPlusVTRComptcdSupported() (NewPlusVTRComptcdSupported bool, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewPlusVTRComptcdSupported string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANPOTSLinkConfig_1, "GetPlusVTRComptcdSupported", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewPlusVTRComptcdSupported, err = soap.UnmarshalBoolean(response.NewPlusVTRComptcdSupported); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
