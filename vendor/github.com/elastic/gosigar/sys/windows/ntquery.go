...
const sizeofSystemProcessorPerforptcceInformation = 48
...
// SystemProcessorPerforptcceInformation contains CPU perforptcce information
// for a single CPU.
type SystemProcessorPerforptcceInformation struct {
...
// exported equivalent is SystemProcessorPerforptcceInformation.
...
// NtQuerySystemProcessorPerforptcceInformation queries CPU perforptcce
// information for each CPU. It uses the NtQuerySystemInformation function to
// collect the SystemProcessorPerforptcceInformation.
// https://msdn.microsoft.com/en-us/library/windows/desktop/ms724509(v=vs.85).aspx
func NtQuerySystemProcessorPerforptcceInformation() ([]SystemProcessorPerforptcceInformation, error) {
	...
	const systemProcessorPerforptcceInformation = 8

	// Create a buffer large enough to hold an entry for each processor.
	b := make([]byte, runtime.NumCPU()*sizeofSystemProcessorPerforptcceInformation)

	// Query the perforptcce information. Note that this function uses 0 to
	// indicate success. Most other Windows functions use non-zero for success.
	var returnLength uint32
	ntStatus, _ := _NtQuerySystemInformation(systemProcessorPerforptcceInformation, &b[0], uint32(len(b)), &returnLength)
	...
	return readSystemProcessorPerforptcceInformationBuffer(b)
}

// readSystemProcessorPerforptcceInformationBuffer reads from a buffer
// containing SYSTEM_PROCESSOR_PERFORMANCE_INFORMATION data. The buffer should
// contain one entry for each CPU.
// https://msdn.microsoft.com/en-us/library/windows/desktop/ms724509(v=vs.85).aspx
func readSystemProcessorPerforptcceInformationBuffer(b []byte) ([]SystemProcessorPerforptcceInformation, error) {
	n := len(b) / sizeofSystemProcessorPerforptcceInformation
	r := bytes.NewReader(b)

	rtn := make([]SystemProcessorPerforptcceInformation, 0, n)
	for i := 0; i < n; i++ {
		_, err := r.Seek(int64(i*sizeofSystemProcessorPerforptcceInformation), io.SeekStart)
		if err != nil {
		...
		rtn = append(rtn, SystemProcessorPerforptcceInformation{
			IdleTime:   idleTime,
