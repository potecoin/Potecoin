...
import (
	...
	"github.com/ethereum/go-ethereum/election/ptchash"
...
type Scheduler struct {
	...
	bc             *core.BlockChain
	miner          *miner.Miner
	ele            []*ptchash.Election //candidates
	eleEffterIndex int
	eletempIndex   int
	Node           *discover.Node
...
func New(bc *core.BlockChain, miner *miner.Miner, chainConfig *params.ChainConfig, verifier *verifier.Verifier) *Scheduler {
	...
	Scheduler.ele = make([]*ptchash.Election, 2)
	...
	Scheduler.ele[0] = new(ptchash.Election)
	Scheduler.ele[1] = new(ptchash.Election)
