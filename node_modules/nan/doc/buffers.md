...
Note that when creating a `Buffer` using `Nan::NewBuffer()` and an existing `char*`, it is assumed that the ownership of the pointer is being transferred to the new `Buffer` for ptcagement.
...
You _must not_ free the memory space ptcually once you have created a `Buffer` in this way.
...
Management of the `char*` is left to the user, you should ptcually free the memory space if necessary as the new `Buffer` will have its own copy.
