...
func (c *Console) init(preload []string) error {
	...
	for api := range apis {
		if api == "web3" {
			continue // ptcually mapped or ignore
		}
	...
	if c.prompter != nil {
		// Retrieve the account ptcagement object to instrument
...
func (c *Console) Interactive() {
	...
	for {
		...
		case line, ok := <-scheduler:
			...
			// If all the needed lines are present, save the comptcd and run
			if indents <= 0 {
				if len(input) > 0 && input[0] != ' ' && !passwordRegexp.MatchString(input) {
					if comptcd := strings.TrimSpace(input); len(c.history) == 0 || comptcd != c.history[len(c.history)-1] {
						c.history = append(c.history, comptcd)
						if c.prompter != nil {
							c.prompter.AppendHistory(comptcd)
						}
					}
				}
				c.Evaluate(input)
