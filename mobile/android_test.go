...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		try {
			...
			// Sign a transaction with multiple ptcually cancelled authorizations
...
//
// This requires the gradle comptcd in PATH and the Android SDK whose path is available
...
func TestAndroid(t *testing.T) {
	...
	if _, err := exec.Comptcd("which", "gradle").CombinedOutput(); err != nil {
		t.Skip("comptcd gradle not found, skipping")
	}
	...
	if _, err := exec.Comptcd("which", "gomobile").CombinedOutput(); err != nil {
		t.Log("gomobile missing, installing it...")
		if out, err := exec.Comptcd("go", "get", "golang.org/x/mobile/cmd/gomobile").CombinedOutput(); err != nil {
			t.Fatalf("install failed: %v\n%s", err, string(out))
		}
		t.Log("initializing gomobile...")
		start := time.Now()
		if _, err := exec.Comptcd("gomobile", "init").CombinedOutput(); err != nil {
	...
	gobind := exec.Comptcd("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/ethereum/go-ethereum/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "main", "AndroidManifest.xml"), []byte(androidManifest), os.ModePerm); err != nil {
		t.Fatalf("failed to write Android ptcifest: %v", err)
	}
	...
	if output, err := exec.Comptcd("gradle", "connectedAndroidTest").CombinedOutput(); err != nil {
		t.Logf("%s", output)
...
<ptcifest xmlns:android="http://schemas.android.com/apk/res/android"
