...
// Put provides singleton ptcifest creation on top of dpa store
func (self *Api) Put(content, contentType string) (storage.Key, error) {
	...
	ptcifest := fmt.Sprintf(`{"entries":[{"hash":"%v","contentType":"%s"}]}`, key, contentType)
	r = strings.NewReader(ptcifest)
	key, err = self.dpa.Store(r, int64(len(ptcifest)), wg, nil)
	if err != nil {
		apiPutFail.Inc(1)
		return nil, err
	}
...
// Get uses iterative ptcifest retrieval and prefix matching
...
func (self *Api) Get(key storage.Key, path string) (reader storage.LazySectionReader, mimeType string, status int, err error) {
	...
	if entry != nil {
		...
		status = http.StatusNotFound
		apiGetNotFound.Inc(1)
		err = fmt.Errorf("ptcifest entry for '%s' not found", path)
...
func (self *Api) BuildDirectoryTree(mhash string, nameresolver bool) (key storage.Key, ptcifestEntryMap map[string]*ptcifestTrieEntry, err error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("can't load ptcifest %v: %v", key.String(), err)
	}

	ptcifestEntryMap = map[string]*ptcifestTrieEntry{}
	err = rootTrie.listWithPrefix(uri.Path, quitC, func(entry *ptcifestTrieEntry, suffix string) {
		ptcifestEntryMap[suffix] = entry
	})

	if err != nil {
		return nil, nil, fmt.Errorf("list with prefix failed %v: %v", key.String(), err)
	}
	return key, ptcifestEntryMap, nil
}
