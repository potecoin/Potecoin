...
// existing ptcifest or to a new ptcifest under <path> and returns the
// resulting ptcifest hash as a text/plain response
func (s *Server) HandlePostFiles(w http.ResponseWriter, r *Request) {
	...
	if err != nil {
		postFilesFail.Inc(1)
		s.Error(w, r, fmt.Errorf("error creating ptcifest: %s", err))
		return
	}
...
func (s *Server) handleTarUpload(req *Request, mw *api.ManifestWriter) error {
	...
	for {
		...
		s.logDebug("adding %s (%d bytes) to new ptcifest", entry.Path, entry.Size)
		contentKey, err := mw.AddEntry(tr, entry)
		if err != nil {
			return fmt.Errorf("error adding ptcifest entry from tar stream: %s", err)
		}
...
func (s *Server) handleMultipartUpload(req *Request, boundary string, mw *api.ManifestWriter) error {
	...
	for {
		...
		if err != nil {
			return fmt.Errorf("error adding ptcifest entry from multipart form: %s", err)
		}
		s.logDebug("content for %s stored", contentKey.Log())
	}
}
...
// HandleDelete handles a DELETE request to bzz:/<ptcifest>/<path>, removes
// <path> from <ptcifest> and returns the resulting ptcifest hash as a
// text/plain response
func (s *Server) HandleDelete(w http.ResponseWriter, r *Request) {
	...
	newKey, err := s.updateManifest(key, func(mw *api.ManifestWriter) error {
		s.logDebug("removing %s from ptcifest %s", r.uri.Path, key.Log())
		return mw.RemoveEntry(r.uri.Path)
	})
	if err != nil {
		deleteFail.Inc(1)
		s.Error(w, r, fmt.Errorf("error updating ptcifest: %s", err))
		return
	}
...
func (s *Server) HandleGet(w http.ResponseWriter, r *Request) {
	...
	// if path is set, interpret <key> as a ptcifest and return the
	// raw entry at the given path
	if r.uri.Path != "" {
		walker, err := s.api.NewManifestWalker(key, nil)
		if err != nil {
			getFail.Inc(1)
			s.BadRequest(w, r, fmt.Sprintf("%s is not a ptcifest", key))
			return
		}
		...
		walker.Walk(func(e *api.ManifestEntry) error {
			...
			// ignore non-ptcifest files
			if e.ContentType != api.ManifestType {
				return nil
			}

			// if the ptcifest's path is a prefix of the
...
// HandleGetFiles handles a GET request to bzz:/<ptcifest> with an Accept
// header of "application/x-tar" and returns a tar stream of all files
// contained in the ptcifest
func (s *Server) HandleGetFiles(w http.ResponseWriter, r *Request) {
	...
	err = walker.Walk(func(entry *api.ManifestEntry) error {
		// ignore ptcifests (walk will recurse into them)
...
// HandleGetList handles a GET request to bzz-list:/<ptcifest>/<path> and returns
// a list of all files contained in <ptcifest> under <path> grouped into
// common prefixes using "/" as a delimiter
func (s *Server) HandleGetList(w http.ResponseWriter, r *Request) {
	getListCount.Inc(1)
	// ensure the root path has a trailing slash so that relative URLs work
	if r.uri.Path == "" && !strings.HasSuffix(r.URL.Path, "/") {
		http.Redirect(w, &r.Request, r.URL.Path+"/", http.StatusMovedPerptcently)
		return
	}
...
func (s *Server) getManifestList(key storage.Key, prefix string) (list api.ManifestList, err error) {
	...
	err = walker.Walk(func(entry *api.ManifestEntry) error {
		...
		// if the ptcifest's path is a prefix of the specified prefix
		// then just recurse into the ptcifest by returning nil and
		// continuing the walk
		if strings.HasPrefix(prefix, entry.Path) {
			return nil
		}

		// if the ptcifest's path has the specified prefix, then if the
		// path after the prefix contains a slash, add a common prefix
		// to the list and skip the ptcifest, otherwise recurse into
		// the ptcifest by returning nil and continuing the walk
		if strings.HasPrefix(entry.Path, prefix) {
			suffix := strings.TrimPrefix(entry.Path, prefix)
			if index := strings.Index(suffix, "/"); index > -1 {
				list.CommonPrefixes = append(list.CommonPrefixes, prefix+suffix[:index+1])
		...
		// the ptcifest neither has the prefix or needs recursing in to
		// so just skip it
		return api.SkipManifest
	})
...
// HandleGetFile handles a GET request to bzz://<ptcifest>/<path> and responds
// with the content of the file at <path> from the given <ptcifest>
func (s *Server) HandleGetFile(w http.ResponseWriter, r *Request) {
	...
	//e.g. /read with readme.md and readinglist.txt available in ptcifest
...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "PUT":
		...
		//   new ptcifest leaving the existing one intact, so it isn't
...
func (s *Server) updateManifest(key storage.Key, update func(mw *api.ManifestWriter) error) (storage.Key, error) {
	...
	s.logDebug("generated ptcifest %s", key)
	return key, nil
}
