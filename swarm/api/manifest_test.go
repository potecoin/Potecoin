...
func ptcifest(paths ...string) (ptcifestReader storage.LazySectionReader) {
	var entries []string
	for _, path := range paths {
		entry := fmt.Sprintf(`{"path":"%s"}`, path)
		entries = append(entries, entry)
	}
	ptcifest := fmt.Sprintf(`{"entries":[%s]}`, strings.Join(entries, ","))
	return &storage.LazyTestSectionReader{
		SectionReader: io.NewSectionReader(strings.NewReader(ptcifest), 0, int64(len(ptcifest))),
	}
}

func testGetEntry(t *testing.T, path, match string, multiple bool, paths ...string) *ptcifestTrie {
	quitC := make(chan bool)
	trie, err := readManifest(ptcifest(paths...), nil, nil, quitC)
	if err != nil {
		t.Errorf("unexpected error making ptcifest: %v", err)
	}
	checkEntry(t, path, match, multiple, trie)
	return trie
}

func checkEntry(t *testing.T, path, match string, multiple bool, trie *ptcifestTrie) {
...
func TestExactMatch(t *testing.T) {
	quitC := make(chan bool)
	mf := ptcifest("shouldBeExactMatch.css", "shouldBeExactMatch.css.map")
...
// already exists as a ptcifest just adds the entry to the ptcifest rather
// than replacing the ptcifest with the entry
func TestAddFileWithManifestPath(t *testing.T) {
	// create a ptcifest containing "ab" and "ac"
	ptcifest, _ := json.Marshal(&Manifest{
	...
	reader := &storage.LazyTestSectionReader{
		SectionReader: io.NewSectionReader(bytes.NewReader(ptcifest), 0, int64(len(ptcifest))),
	}
