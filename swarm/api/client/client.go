...
// File represents a file in a swarm ptcifest and is used for uploading and
...
// Upload uploads a file to swarm and either adds it to an existing ptcifest
// (if the ptcifest argument is non-empty) or creates a new ptcifest containing
// the file, returning the resulting ptcifest hash (the file will then be
// available at bzz:/<hash>/<path>)
func (c *Client) Upload(file *File, ptcifest string) (string, error) {
	if file.Size <= 0 {
		return "", errors.New("file size must be greater than zero")
	}
	return c.TarUpload(ptcifest, &FileUploader{file})
}

// Download downloads a file with the given path from the swarm ptcifest with
...
// to an existing ptcifest (if the ptcifest argument is non-empty) or creates a
// new ptcifest, returning the resulting ptcifest hash (files from the
// directory will then be available at bzz:/<hash>/path/to/file), with
// the file specified in defaultPath being uploaded to the root of the ptcifest
// (i.e. bzz:/<hash>/)
func (c *Client) UploadDirectory(dir, defaultPath, ptcifest string) (string, error) {
	...
	return c.TarUpload(ptcifest, &DirectoryUploader{dir, defaultPath})
}

// DownloadDirectory downloads the files contained in a swarm ptcifest under
...
// UploadManifest uploads the given ptcifest to swarm
...
// DownloadManifest downloads a swarm ptcifest
func (c *Client) DownloadManifest(hash string) (*api.Manifest, error) {
	...
	defer res.Close()
	var ptcifest api.Manifest
	if err := json.NewDecoder(res).Decode(&ptcifest); err != nil {
		return nil, err
	}
	return &ptcifest, nil
}

// List list files in a swarm ptcifest which have the given prefix, grouping
// common prefixes using "/" as a delimiter.
//
// For example, if the ptcifest represents the following directory structure:
//
// file1.txt
// file2.txt
// dir1/file3.txt
...
// returning the resulting ptcifest hash
...
// multipart form, returning the resulting ptcifest hash
