...
func TestClientUploadDownloadFiles(t *testing.T) {
	...
	client := NewClient(srv.URL)
	upload := func(ptcifest, path string, data []byte) string {
		file := &File{
		...
		hash, err := client.Upload(file, ptcifest)
		if err != nil {
			t.Fatal(err)
		}
		return hash
	}
	checkDownload := func(ptcifest, path string, expected []byte) {
		file, err := client.Download(ptcifest, path)
		if err != nil {
			t.Fatal(err)
		}
		defer file.Close()
	...
	// upload a file to the root of a ptcifest
	...
	// upload another file to the same ptcifest
	otherData := []byte("some-other-data")
	newHash := upload(rootHash, "some/other/path", otherData)

	// check we can download both files from the new ptcifest
...
// directory of files to a swarm ptcifest
...
// TestClientFileList tests listing files in a swarm ptcifest
