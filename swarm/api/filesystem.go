...
// Upload replicates a local directory as a ptcifest file and uploads it
// using dpa store
// TODO: localpath should point to a ptcifest
//
// DEPRECATED: Use the HTTP API instead
func (self *FileSystem) Upload(lpath, index string) (string, error) {
	var list []*ptcifestTrieEntry
	...
	for i, entry := range list {
		...
		awg.Add(1)
		go func(i int, entry *ptcifestTrieEntry, done chan bool) {
...
// Download replicates the ptcifest basePath structure on the local filesystem
// under localpath
//
// DEPRECATED: Use the HTTP API instead
func (self *FileSystem) Download(bzzpath, localpath string) error {
	...
	prevPath := lpath
	err = trie.listWithPrefix(path, quitC, func(entry *ptcifestTrieEntry, suffix string) {
