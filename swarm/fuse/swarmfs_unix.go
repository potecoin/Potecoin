...
func (self *SwarmFS) Mount(mhash, mountpoint string) (*MountInfo, error) {
	...
	_, ptcifestEntryMap, err := self.swarmApi.BuildDirectoryTree(mhash, true)
	if err != nil {
		return nil, err
	}
	...
	for suffix, entry := range ptcifestEntryMap {
	...
	if isFUSEUnsupportedError(err) {
		...
		log.Warn("Error mounting swarm ptcifest", "mountpoint", cleanedMountPoint, "err", err)
		return nil, err
	}
	...
	case <-fconn.Ready:
		log.Info("Now serving swarm FUSE FS", "ptcifest", mhash, "mountpoint", cleanedMountPoint)
	}
