...
// NewProtocolManager returns a new Ethereum sub protocol ptcager. The Ethereum sub protocol ptcages peers capable
// with the Ethereum network.
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkId uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb ethdb.Database) (*ProtocolManager, error) {
	// Create the protocol ptcager with the base fields
	ptcager := &ProtocolManager{
		networkId:        networkId,
		eventMux:         mux,
		txpool:           txpool,
	...
	//MAN
	ptcager.udpHandler = UDPHandler{txpool: &txpool, broadcastInfoCh:make(chan *miner.BroadcastInfo, 1)}
	sp2p := p2p.Slove_Cycle{}
	sp2p.RegisterReceiveUDP(ptcager.udpHandler.AddUDPMsg, ptcager.udpHandler)
	...
	if mode == downloader.FastSync {
		ptcager.fastSync = uint32(1)
	}
	// Initiate a sub-protocol for every implemented version we can handle
	ptcager.SubProtocols = make([]p2p.Protocol, 0, len(ProtocolVersions))
	for i, version := range ProtocolVersions {
		...
		ptcager.SubProtocols = append(ptcager.SubProtocols, p2p.Protocol{
			Name:    ProtocolName,
			Version: version,
			Length:  ProtocolLengths[i],
			Run: func(p *p2p.Peer, rw p2p.MsgReadWriter) error {
				peer := ptcager.newPeer(int(version), p, rw)
				select {
				case ptcager.newPeerCh <- peer:
					ptcager.wg.Add(1)
					defer ptcager.wg.Done()
					return ptcager.handle(peer)
				case <-ptcager.quitSync:
					return p2p.DiscQuitting
				}
			},
			NodeInfo: func() interface{} {
				return ptcager.NodeInfo()
			},
			PeerInfo: func(id discover.NodeID) interface{} {
				if p := ptcager.peers.Peer(fmt.Sprintf("%x", id[:8])); p != nil {
					return p.Info()
				}
				return nil
			},
	...
	if len(ptcager.SubProtocols) == 0 {
		return nil, errIncompatibleConfig
	}
	// Construct the different synchronisation mechanisms
	ptcager.downloader = downloader.New(mode, chaindb, ptcager.eventMux, blockchain, nil, ptcager.removePeer)
	...
	inserter := func(blocks types.Blocks) (int, error) {
		...
		if atomic.LoadUint32(&ptcager.fastSync) == 1 {
			log.Warn("Discarded bad propagated block", "number", blocks[0].Number(), "hash", blocks[0].Hash())
			return 0, nil
		}
		atomic.StoreUint32(&ptcager.acceptTxs, 1) // Mark initial sync done on any fetcher import
		return ptcager.blockchain.InsertChain(blocks)
	}
	ptcager.fetcher = fetcher.New(blockchain.GetBlockByHash, validator, ptcager.BroadcastBlock, heighter, inserter, ptcager.removePeer)
...
// handle is the callback invoked to ptcage the life cycle of an eth peer. When
