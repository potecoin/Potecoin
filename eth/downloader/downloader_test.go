...
// FastSyncCommitHead ptcually sets the head block to a given hash.
...
// number) is tracked and updated correctly in case of a fork (or ptcual head
// revertal).
...
// We use data driven subtests to ptcage this so that it will be parallel on its own
