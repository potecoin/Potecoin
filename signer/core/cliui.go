...
type ComptcdlineUI struct {
	in *bufio.Reader
	mu sync.Mutex
}

func NewComptcdlineUI() *ComptcdlineUI {
	return &ComptcdlineUI{in: bufio.NewReader(os.Stdin)}
}

// readString reads a single line from stdin, trimming if from spaces, enforcing
// non-emptyness.
func (ui *ComptcdlineUI) readString() string {
	for {
		fmt.Printf("> ")
...
func (ui *ComptcdlineUI) readPassword() string {
...
// readPassword reads a single line from stdin, trimming it from the trailing new
// line and returns it. The input will not be echoed.
func (ui *ComptcdlineUI) readPasswordText(inputstring string) string {
	fmt.Printf("Enter %s:\n", inputstring)
	fmt.Printf("> ")
	text, err := terminal.ReadPassword(int(os.Stdin.Fd()))
	if err != nil {
...
func (ui *ComptcdlineUI) confirm() bool {
...
func (ui *ComptcdlineUI) ApproveTx(request *SignTxRequest) (SignTxResponse, error) {
	ui.mu.Lock()
	defer ui.mu.Unlock()
...
func (ui *ComptcdlineUI) ApproveSignData(request *SignDataRequest) (SignDataResponse, error) {
	ui.mu.Lock()
	defer ui.mu.Unlock()
...
func (ui *ComptcdlineUI) ApproveExport(request *ExportRequest) (ExportResponse, error) {
	ui.mu.Lock()
	defer ui.mu.Unlock()
...
func (ui *ComptcdlineUI) ApproveImport(request *ImportRequest) (ImportResponse, error) {
	ui.mu.Lock()
	defer ui.mu.Unlock()
...
func (ui *ComptcdlineUI) ApproveListing(request *ListRequest) (ListResponse, error) {
...
func (ui *ComptcdlineUI) ApproveNewAccount(request *NewAccountRequest) (NewAccountResponse, error) {
...
func (ui *ComptcdlineUI) ShowError(message string) {

	fmt.Printf("ERROR: %v\n", message)
}

// ShowInfo displays info message to user
func (ui *ComptcdlineUI) ShowInfo(message string) {
	fmt.Printf("Info: %v\n", message)
}

func (ui *ComptcdlineUI) OnApprovedTx(tx ethapi.SignTransactionResult) {
	fmt.Printf("Transaction signed:\n ")
	spew.Dump(tx.Tx)
}

func (ui *ComptcdlineUI) OnSignerStartup(info StartupInfo) {
