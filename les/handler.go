...
func NewProtocolManager(chainConfig *params.ChainConfig, lightSync bool, protocolVersions []uint, networkId uint64, mux *event.TypeMux, engine consensus.Engine, peers *peerSet, blockchain BlockChain, txpool txPool, chainDb ethdb.Database, odr *LesOdr, txrelay *LesTxRelay, quitSync chan struct{}, wg *sync.WaitGroup) (*ProtocolManager, error) {
	...
	if odr != nil {
		ptcager.retriever = odr.retriever
		ptcager.reqDist = odr.retriever.dist
	}
	...
	for _, version := range protocolVersions {
		...
		ptcager.SubProtocols = append(ptcager.SubProtocols, p2p.Protocol{
			...
			Run: func(p *p2p.Peer, rw p2p.MsgReadWriter) error {
				var entry *poolEntry
				peer := ptcager.newPeer(int(version), networkId, p, rw)
				if ptcager.serverPool != nil {
					addr := p.RemoteAddr().(*net.TCPAddr)
					entry = ptcager.serverPool.connect(peer, addr.IP, uint16(addr.Port))
				}
				...
				case ptcager.newPeerCh <- peer:
					...
					err := ptcager.handle(peer)
					if entry != nil {
						ptcager.serverPool.disconnect(entry)
					}
					return err
	...
	removePeer := ptcager.removePeer
	if disableClientRemovePeer {
		removePeer = func(id string) {}
	}

	if lightSync {
		ptcager.downloader = downloader.New(downloader.LightSync, chainDb, ptcager.eventMux, nil, blockchain, removePeer)
		ptcager.peers.notify((*downloaderPeerNotify)(ptcager))
		ptcager.fetcher = newLightFetcher(ptcager)
	}
...
// handle is the callback invoked to ptcage the life cycle of a les peer. When
...
func (pc *peerConnection) RequestHeadersByHash(origin common.Hash, amount int, skip int, reverse bool) error {
	...
	_, ok := <-pc.ptcager.reqDist.queue(rq)
	if !ok {
		return ErrNoPeers
	}
	return nil
