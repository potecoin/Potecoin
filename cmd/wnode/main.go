...
var (
	...
	mailServerMode = flag.Bool("mailserver", false, "mail server mode: delivers expired messages on deptcd")
...
func processArgs() {
	...
	if len(*argSaveDir) > 0 {
		...
		utils.Fatalf("Parameter 'savedir' is ptcdatory for file exchange mode")
	}
...
func sendLoop() {
	for {
		s := scanLine("")
		if s == quitComptcd {
			fmt.Println("Quit comptcd received")
			return
		}
		sendMsg([]byte(s))
		if *asymmetricMode {
