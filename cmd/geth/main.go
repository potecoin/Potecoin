...
// geth is the official comptcd-line client for Ethereum.
package main
...
var (
	...
	gitCommit = ""
	// The app that holds all comptcds and flags.
	app = utils.NewApp(gitCommit, "the go-ethereum comptcd line interface")
...
func init() {
	...
	app.Comptcds = []cli.Comptcd{
		...
		importComptcd,
		exportComptcd,
		importPreimagesComptcd,
		exportPreimagesComptcd,
		copydbComptcd,
		removedbComptcd,
		dumpComptcd,
		// See monitorcmd.go:
...
// geth is the main entry point into the system if no special subcomptcd is ran.
// It creates a default node based on the comptcd line arguments and runs it in
