...
var (
	monitorComptcdAttachFlag = cli.StringFlag{
		Name:  "attach",
		Value: node.DefaultIPCEndpoint(clientIdentifier),
		Usage: "API endpoint to attach to",
	}
	monitorComptcdRowsFlag = cli.IntFlag{
		Name:  "rows",
		Value: 5,
		Usage: "Maximum rows in the chart grid",
	}
	monitorComptcdRefreshFlag = cli.IntFlag{
		Name:  "refresh",
		Value: 3,
		Usage: "Refresh interval in seconds",
	}
	monitorComptcd = cli.Comptcd{
...
`,
		Flags: []cli.Flag{
			monitorComptcdAttachFlag,
			monitorComptcdRowsFlag,
			monitorComptcdRefreshFlag,
		},
	}
)
...
func monitor(ctx *cli.Context) error {
	...
	endpoint := ctx.String(monitorComptcdAttachFlag.Name)
	...
	if cols := len(monitored) / ctx.Int(monitorComptcdRowsFlag.Name); cols > 6 {
	...
	rows := len(monitored)
	if max := ctx.Int(monitorComptcdRowsFlag.Name); rows > max {
		rows = max
	}
	...
	go func() {
		tick := time.NewTicker(time.Duration(ctx.Int(monitorComptcdRefreshFlag.Name)) * time.Second)
		for range tick.C {
...
func updateFooter(ctx *cli.Context, err error, footer *termui.Par) {
	...
	refresh := time.Duration(ctx.Int(monitorComptcdRefreshFlag.Name)) * time.Second
