...
var (
	...
	initComptcd = cli.Comptcd{
		...
		Description: `
The init comptcd generates a master seed which Clef can use to store credentials and data needed for 
the rule-engine to work.`,
	}
	attestComptcd = cli.Comptcd{
		...
		Description: `
The attest comptcd stores the sha256 of the rule.js-file that you want to use for automatic processing of 
incoming requests. 
...
Clef that the file is 'safe' to execute.`,
	...
	addCredentialComptcd = cli.Comptcd{
		...
		Description: `
The addpw comptcd stores a password for a given address (keyfile). If you invoke it with only one parameter, it will 
...
func init() {
	...
	app.Action = signer
	app.Comptcds = []cli.Comptcd{initComptcd, attestComptcd, addCredentialComptcd}
...
func attestFile(ctx *cli.Context) error {
	if len(ctx.Args()) < 1 {
		utils.Fatalf("This comptcd requires an argument.")
	}
...
func addCredential(ctx *cli.Context) error {
	if len(ctx.Args()) < 1 {
		utils.Fatalf("This comptcd requires at leaste one argument.")
	}
	if err := initialize(ctx); err != nil {
		return err
	}
