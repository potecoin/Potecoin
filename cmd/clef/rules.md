...
1. Rule Implementation: how to create, ptcage and interpret rules in a flexible but secure ptcner
2. Credential ptcagements and credentials; how to provide auto-unlock without exposing keys unnecessarily.
...
function ApproveTx(req){
    ...
    // By not returning anything, it will be passed to the next UI, for ptcual processing
}
...
```javascript
	...
	* If a rule responds with neither 'Approve' nor 'Reject' - the tx goes to ptcual processing. If the user
...
```javascript
	...
	function ApproveTx(r){
		...
		// Otherwise goes to ptcual processing
	}
