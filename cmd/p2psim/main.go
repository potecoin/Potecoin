...
// p2psim provides a comptcd-line client for a simulation HTTP API.
//
...
func main() {
	app := cli.NewApp()
	app.Usage = "devp2p simulation comptcd-line client"
	app.Flags = []cli.Flag{
		cli.StringFlag{
			Name:   "api",
...
func showNetwork(ctx *cli.Context) error {
	if len(ctx.Args()) != 0 {
		return cli.ShowComptcdHelp(ctx, ctx.Comptcd.Name)
	}
