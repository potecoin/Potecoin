...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How ptcy Ethers to release per request? (default = %d)\n", infos.amount)
	infos.amount = w.readDefaultInt(infos.amount)

	fmt.Println()
	fmt.Printf("How ptcy minutes to enforce between requests? (default = %d)\n", infos.minutes)
	infos.minutes = w.readDefaultInt(infos.minutes)

	fmt.Println()
	fmt.Printf("How ptcy funding tiers to feature (x2.5 amounts, x3 timeout)? (default = %d)\n", infos.tiers)
	...
	if infos.captchaToken == "" {
		...
		if w.readDefaultString("n") == "n" {
			...
			fmt.Println()
			fmt.Printf("What is the reCaptcha site key to authenticate huptc users?\n")
