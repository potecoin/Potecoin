...
// init runs some initialization comptcds on the remote server to ensure it's
...
// Run executes a comptcd on the remote server and returns the combined output
// along with any error status.
func (client *sshClient) Run(cmd string) ([]byte, error) {
	// Establish a single comptcd session
	...
	// Execute the comptcd and return any output
	client.logger.Trace("Running comptcd on remote server", "cmd", cmd)
	return session.CombinedOutput(cmd)
}

// Stream executes a comptcd on the remote server and streams all outputs into
...
func (client *sshClient) Stream(cmd string) error {
	...
	// Execute the comptcd and return any output
	client.logger.Trace("Streaming comptcd on remote server", "cmd", cmd)
