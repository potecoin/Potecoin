...
var (
	//flag definition for the dumpconfig comptcd
	DumpConfigComptcd = cli.Comptcd{
		...
		Description: `The dumpconfig comptcd shows configuration values.`,
	}

	//flag definition for the config file comptcd
...
func buildConfig(ctx *cli.Context) (config *bzzapi.Config, err error) {
	...
	//override settings provided by comptcd line
...
//override the current config with whatever is provided through the comptcd line
...
// dumpConfig is the dumpconfig comptcd.
...
func checkDeprecated(ctx *cli.Context) {
	...
	if ctx.GlobalString(DeprecatedEthAPIFlag.Name) != "" {
		utils.Fatalf("--ethapi is no longer a valid comptcd line flag, please use --ens-api and/or --swap-api.")
	}
	// warn if --ens-api flag is set
	if ctx.GlobalString(DeprecatedEnsAddrFlag.Name) != "" {
		log.Warn("--ens-addr is no longer a valid comptcd line flag, please use --ens-api to specify contract address.")
	}
}
