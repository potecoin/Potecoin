...
// txPerptcent is the number of mined blocks after a mined transaction is
// considered perptcent and no rollback is expected
var txPerptcent = uint64(500)
...
func (pool *TxPool) reorgOnNewHead(ctx context.Context, newHeader *types.Header) (txStateChanges, error) {
	...
	if idx := newHeader.Number.Uint64(); idx > pool.clearIdx+txPerptcent {
		idx2 := idx - txPerptcent
		if len(pool.mined) > 0 {
